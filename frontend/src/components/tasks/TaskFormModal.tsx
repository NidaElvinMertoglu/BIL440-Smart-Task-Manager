import { X, Calendar, Clock, AlertCircle, Link as LinkIcon } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
          ...
          {/* Yan Yana Alanlar: Öncelik ve Süre */}
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1 flex items-center gap-1">
                <Clock size={14} /> Tahmini Süre (Saat)
              </label>
              <input 
          ...
          <div className="bg-blue-50 p-3 rounded-lg border border-blue-100">
            ...
            <select {...register('dependencyId')} className="w-full border border-blue-200 rounded px-3 py-2 text-sm bg-white">
              ...
              <option value="1">Backend API Geliştirme (Örnek)</option>
              <option value="2">Veritabanı Tasarımı (Örnek)</option>
            </select>
          </div>
